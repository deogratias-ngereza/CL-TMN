...
APP.controller('TabulationReqController', ['$timeout','$scope','$state','$stateParams','AuthService','CookieManagerService','GServices','HelperService','GDumpService','DTOptionsBuilder', 'DTColumnDefBuilder',
 function($timeout,$scope,$state,$stateParams,AuthService,CookieManagerService,GServices,HelperService,GDumpService,DTOptionsBuilder, DTColumnDefBuilder) {
    ...
    GDumpService.log(0, -1,"TabulationReqController"); 
    ...
    $scope.VENDORS_LIST = [];//CookieManagerService.getVendorsList();
   ...
   //
    $scope.openDutyAndTaxPaymentListModal = function(data){
        ...
        $scope.PICKED_OBJ.dnt_payment = {
            ...
            consignee : $scope.PICKED_OBJ.consignee_company == ''?$scope.PICKED_OBJ.consignee_name:$scope.PICKED_OBJ.consignee_company,
    ...
    */
    $scope.get_shipments_tabulations = function(){
      	$scope.change_loading_status(true,"loading tabulations ...");
     	var conn = GServices.get_tabulation_requests_filter($scope.FILTER_OPTION);
	    conn.then(
	        function(response){
	            $scope.IS_LOADING = false;
	            if(response.status == 200 || 201 || 202 || 203 || 204){ 
